...
void solve()
{
    ...
    map<int, vi> m;
    rep(i, 0, n) m[v[i]].pb(i + 1);
    fore(e, m) SORT(e.second);

    vi ans(n);
    fore(e, m)
    {
        int i = 0, j = e.second.size() - 1;
        if (i == j)
        {
            cout << -1 << endl;
            return;
        ...
        while (i <= j)
        {
            ans[e.second[j] - 1] = e.second[i];
            ans[e.second[i] - 1] = e.second[j];
            i++;
            j--;
        }
        ...
        i = 0, j = e.second.size();
        if (j % 2 != 0)
            swap(ans[e.second[j / 2] - 1], ans[e.second[0] - 1]);
    }
