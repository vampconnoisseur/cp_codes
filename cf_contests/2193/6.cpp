...
void solve()
{
    ...
    rep(i, 0, n)
    {
        ...
        {
            mp[x[i]].X = min(mp[x[i]].X, y[i]);
            mp[x[i]].Y = max(mp[x[i]].Y, y[i]);
        }
    }

    auto it = mp.begin();
    ll prev_x = it->first;
    ll prev_low = it->second.X;
    ll prev_high = it->second.Y;
    ...
    {
        ll cur_x = it->first;
        ll cur_low = it->second.X;
        ll cur_high = it->second.Y;

        ll dist_x = cur_x - prev_x;
        ll dist_y = cur_high - cur_low;

        ll cost_to_low = min(dp_low + abs(prev_low - cur_high), dp_high + abs(prev_high - cur_high)) + dist_x + dist_y;
        ll cost_to_high = min(dp_low + abs(prev_low - cur_low), dp_high + abs(prev_high - cur_low)) + dist_x + dist_y;

        dp_low = cost_to_low;
        dp_high = cost_to_high;
