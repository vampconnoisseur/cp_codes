...
void solve()
{
    ...
    ll canSolve = upper_bound(all(b), kevin) - b.begin();
    ll cannotSolve = m - canSolve;

    vll vec;

    rep(i, canSolve, m)
    {
        ll cnt = a.end() - lower_bound(all(a), b[i]);
        vec.push_back(cnt);
    }
    ...
    rep(k, 1, m + 1)
    {
        ...
        ll fullKevinGroups = canSolve / k;
        ans += fullKevinGroups;

        ll remKevin = canSolve % k;
        ...
        while (idx + k <= vec.size())
        {
            ll maxVal = vec[idx + k - 1];
            ans += maxVal + 1;
            idx += k;
        }
