...
void solve()
{
    ...
    while (start < n)
    {
        ...
        auto calcLogSum = [&](int L, int R) -> long double
        {
            if (L > R)
                return -1.0L;
            long double sum = 0;
        ...
        if (negCount % 2 == 0)
        {
            long double curLog = calcLogSum(l, r);
            if (curLog > bestLog)
            {
                bestLog = curLog;
                bestL = l;
        ...
        {
            long double log1 = calcLogSum(negPos[0] + 1, r);
            long double log2 = calcLogSum(l, negPos.back() - 1);
