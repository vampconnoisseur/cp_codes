...
void solve()
{
    ...
    vll a = {-2000000000000000000LL};
    a.resize(n + 1);
    for (int i = 1; i <= n; ++i)
        cin >> a[i];
    a.pb(2000000000000000000LL);

    SORT(a);

    auto traverse = [&](ll mid, auto op)
    {
        for (size_t i = 0; i < a.size() - 1; ++i)
        {
            ll L = max(0LL, a[i] + mid);
            ll R = min(x, a[i + 1] - mid);
    ...
    {
        traverse(ans, [&](ll L, ll R)
                 {
            for (ll p = L; p <= R && k > 0; ++p) {
                cout << p << " ";
                k--;
            }
            return k > 0; });
