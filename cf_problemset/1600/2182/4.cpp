...
void solve()
{
    ...
    int highest = 0;
    rep(i, 1, n + 1) highest = max(highest, a[i]);
    ...
    int highestCount = n - nonHighest;

    ll ans = 0;

    rep(i, 0, nonHighest + 1)
    {
        if (sum <= a[0] + i)
        {
            ll ways = combination(nonHighest, i);
            ways = (ways * fact[i]) % MOD;
            ways = (ways * highestCount) % MOD;
            ways = (ways * fact[n - i - 1]) % MOD;
            ans = (ans + ways) % MOD;
        }
    }
