...
void solve()
{
    ...
    vpll fact_sum(1 << fact.size());
    fact_sum[0] = {0, 0};
    rep(mask, 1, (1 << fact.size()))
    {
        int first_bit = get_first_bit(mask);
        fact_sum[mask].X = fact_sum[mask ^ (1 << first_bit)].X + fact[first_bit];
        fact_sum[mask].Y = get_bit_count(mask);
    }
    ...
    fore(p, fact_sum)
    {
        if (p.X <= n)
        {
            res = min(res, p.Y + get_bit_count(n - p.X));
        }
    }
    cout << res << endl;
}
