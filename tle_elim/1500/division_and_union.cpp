...
void solve()
{
    ...
    vpii segments(n);
    vi ans(n);
    rep(i, 0, n) cin >> segments[i];

    vector<pair<pii, int>> seg_with_idx(n);
    rep(i, 0, n) seg_with_idx[i] = {segments[i], i};

    SORT(seg_with_idx);

    vi prefix_max_r(n);
    prefix_max_r[0] = seg_with_idx[0].X.Y;
    rep(i, 1, n) prefix_max_r[i] = max(prefix_max_r[i - 1], seg_with_idx[i].X.Y);

    int split = -1;
    rep(i, 0, n - 1)
    {
        if (prefix_max_r[i] < seg_with_idx[i + 1].X.X)
        {
            split = i;
            break;
        }
    ...
    {
        if (i <= split)
            res[seg_with_idx[i].Y] = 1;
        else
            res[seg_with_idx[i].Y] = 2;
    }
