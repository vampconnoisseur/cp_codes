...
void solve()
{
    ...
    auto getId = [&](int x, int y, int dir, int cnt)
    {
        return ((((x * m + y) * 5 + dir) * 4) + cnt);
    };
    ...
    dist[getId(S.X, S.Y, NONE, 0)] = 0;
    q.push({S.X, S.Y, NONE, 0});
    ...
    while (!q.empty())
    {
        ...
        int curDist = dist[getId(x, y, dir, cnt)];
        ...
        rep(ndir, 0, 4)
        {
            ...
            int nid = getId(nx, ny, ndir, ncnt);

            if (dist[nid] != -1)
                continue;

            dist[nid] = curDist + 1;
            q.push({nx, ny, ndir, ncnt});
        }
    }
