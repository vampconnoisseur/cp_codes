...
void solve()
{
    ...
    map<ll, ll> freq;
    vvll g(n, vector<ll>(m, 0));

    int D = m - k;
    ll cap_row = min((ll)D + 1, (ll)k);

    int l = 0, r = m - 1, val = 1;
    while (l <= r)
    {
        ll put = (val <= cap_row ? val : cap_row);
    ...
    D = n - k;
    ll cap_col = min((ll)D + 1, (ll)k);

    l = 0, r = n - 1, val = 1;
    while (l <= r)
    {
        ll put = (val <= cap_col ? val : cap_col);
