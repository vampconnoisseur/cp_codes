...
void solve()
{
    ...
    rep(i, 1, n) freq[i] += freq[i - 1];

    multiset<int, greater<int>> values(all(a));
    ...
    for (auto it = use_count.rbegin(); it != use_count.rend(); ++it)
    {
        int use = it->X;
        int count = it->Y;

        while (count-- && !values.empty())
        {
            ans += 1LL * use * (*values.begin());
