...
void solve()
{
    ...
    cin >> n >> m;
    vvi v(n, vi(m));
    fore(e, v) fore(a, e) cin >> a;

    map<int, vpii> color;
    rep(i, 0, n) rep(j, 0, m) color[v[i][j]].eb(i, j);

    ll total_distance = 0;

    for (auto &[key, points] : color)
    {
        if (points.size() < 2)
            continue;
        ...
        rep(i, 0, k)
        {
            total_distance += (ll)i * points[i].X - prefix_sum;
            prefix_sum += points[i].X;
        }
        ...
        rep(i, 0, k)
        {
            total_distance += (ll)i * points[i].Y - prefix_sum;
