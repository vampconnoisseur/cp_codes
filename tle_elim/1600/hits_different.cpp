...
void solve()
{
    ...
    while (!q.empty())
    {
        ...
        int nxt1 = node + step + 1;
        int nxt2 = node + step + 2;
        int newStep = step + 1;

        if (nxt1 <= n)
        {
            graph[node].push_back(nxt1);
            revGraph[nxt1].push_back(node);
            long long k1 = pack(nxt1, newStep);
            if (!seen.count(k1))
            {
                seen.insert(k1);
                q.push({nxt1, newStep});
        ...
        if (nxt2 <= n)
        {
            graph[node].push_back(nxt2);
            revGraph[nxt2].push_back(node);
            long long k2 = pack(nxt2, newStep);
            if (!seen.count(k2))
            {
                seen.insert(k2);
                q.push({nxt2, newStep});
