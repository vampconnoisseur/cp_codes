...
void solve()
{
    ...
    vector<string> keys = {"A", "B", "C", "AB", "BC", "AC", "ABC"};

    for (auto &key : keys)
        if (!cost_map.count(key))
            cost_map[key] = INF;

    ll A = cost_map["A"], B = cost_map["B"], C = cost_map["C"], AB = cost_map["AB"], BC = cost_map["BC"], AC = cost_map["AC"], ABC = cost_map["ABC"];

    ll ans = INF;
    ans = min({ans, A + B + C, AB + C, AC + B, BC + A, ABC, AB + BC, BC + AC, AB + AC});
