...
void solve()
{
    ...
    vpll coords(n);
    rep(i, 0, n) cin >> coords[i].X >> coords[i].Y;

    ll minDistA = LLONG_MAX, minDistB = LLONG_MAX;

    rep(i, 0, k)
    {
        minDistA = min(minDistA, manhattan(coords[a], coords[i]));
        minDistB = min(minDistB, manhattan(coords[b], coords[i]));
    }

    ll dist1;
    if (minDistA > LLONG_MAX - minDistB)
        dist1 = LLONG_MAX;
    else
        dist1 = minDistA + minDistB;

    ll dist2 = manhattan(coords[a], coords[b]);
