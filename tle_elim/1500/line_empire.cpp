...
void solve()
{
    ...
    rrep(i, n - 1, 0) suffix[i] = suffix[i + 1] + x[i];

    ll ans = LLONG_MAX;

    rep(i, 0, n)
    {
        ll right_sum = suffix[i + 1];
        ll cost = right_sum * b + (ll)(-n + i + 2) * x[i] * b + x[i] * a;
        ans = min(ans, cost);
    }

    ll prefix_sum = accumulate(all(x), 0LL);
    ans = min(ans, prefix_sum * b);
