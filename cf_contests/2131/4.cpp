...
void solve()
{
    ...
    vi leaf_neighbors_count(n + 1, 0);
    rep(i, 1, n + 1)
    {
        if (deg[i] == 1)
        {
            int neighbor = adj[i][0];
            leaf_neighbors_count[neighbor]++;
        }
    }

    int max_saved_leaves = 0;
    rep(r, 1, n + 1)
    {
        int is_leaf = (deg[r] == 1);
        int saved_count = is_leaf + leaf_neighbors_count[r];
        if (saved_count > max_saved_leaves)
        {
            max_saved_leaves = saved_count;
        }
    }

    cout << total_leaves - max_saved_leaves << endl;
}
