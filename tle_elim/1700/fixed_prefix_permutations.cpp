...
int getLCP(const vi &a, const vi &b)
{
    int res = 0;
...
void solve()
{
    ...
    vvi inv(n, vi(m));
    rep(i, 0, n) rep(j, 0, m) inv[i][a[i][j] - 1] = j + 1;
    ...
    rep(i, 0, n)
    {
        int idx = lower_bound(all(sortedInv), a[i]) - sortedInv.begin();

        int best = 0;

        if (idx < n)
            best = max(best, getLCP(a[i], sortedInv[idx]));

        if (idx > 0)
            best = max(best, getLCP(a[i], sortedInv[idx - 1]));
