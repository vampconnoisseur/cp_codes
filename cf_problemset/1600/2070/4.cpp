...
vi adj[MAXN], level_nodes[MAXN];
int depth[MAXN];
ll dp[MAXN], levelSum[MAXN];
...
void solve()
{
    ...
    rrep(d, maxDepth, 0)
    {
        for (int u : level_nodes[d])
        {
            ...
            {
                ll total = levelSum[d + 1];

                if (u != 1)
                {
                    for (int c : adj[u])
                    {
                        total = (total - dp[c] + MOD) % MOD;
                    }
                }
            ...
            levelSum[d] = (levelSum[d] + dp[u]) % MOD;
        }
    }
