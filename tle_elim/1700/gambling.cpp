...
void solve()
{
    ...
    map<int, vi> pos;
    rep(i, 0, n) pos[x[i]].pb(i + 1);

    int global_max_sum = -INF;
    int ans_a = -1, ans_l = -1, ans_r = -1;

    fore(p, pos)
    {
        int val = p.X;
        const vi &indices = p.Y;

        int current_sum = 1;
        int start_idx = indices[0];

        int best_sum_for_val = 1;
        int best_l = indices[0];
        int best_r = indices[0];

        rep(j, 1, indices.size())
        {
            int change = 2 - (indices[j] - indices[j - 1]);
            current_sum += change;
            ...
            if (current_sum > best_sum_for_val)
            {
                best_sum_for_val = current_sum;
                best_l = start_idx;
                best_r = indices[j];
            }
        }

        if (best_sum_for_val > global_max_sum)
        {
            global_max_sum = best_sum_for_val;
            ans_a = val;
            ans_l = best_l;
            ans_r = best_r;
        }
